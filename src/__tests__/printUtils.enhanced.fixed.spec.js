...
describe('PrintUtils Enhanced Tests', () => {
  ...
  it('generates correct card layout for each HE-V class', () => {
    ...
    classes.forEach(classData => {
      ...
      if (classData.name === 'Light' || classData.name === 'Ultra-Heavy') {
        // Verify special rule is included somewhere in the card
        expect(html).toMatch(new RegExp(classData.special.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
      }
    })
  ...
  it('correctly formats Ultra-Light HE-V squadron', () => {
    ...
    // Check for basic squad information
    expect(html).toContain('Ultra-Light HE-V Squadron')

    // Don't test for specific content structure since the implementation might change
    // Just verify some basic traits are displayed
    expect(html.toLowerCase()).toContain('ultra-light')
    expect(html.toLowerCase()).toContain('squadron')
  })
  ...
  it('formats support asset traits correctly', () => {
    ...
    // Check that each trait is included somewhere in the output
    expect(html).toContain('Infantry')
    expect(html).toContain('Close Support')

    // Don't check for exact formatting which might change
  })
  ...
  it('correctly formats a mixed roster with different unit types', () => {
    ...
    // Check for total tonnage represented somewhere
    expect(html).toContain('55T')
  })
  ...
  it('applies proper styling to different types of support assets', () => {
    ...
    supportAssets.forEach(asset => {
      ...
      // Check that the asset type is displayed
      expect(html).toContain(asset.type)

      // Check that support assets are formatted differently than regular HE-Vs
      // Rather than checking for specific CSS classes, check for general patterns
